...
- Known non-blocking Homepage v2 notes: shared `<img>` warnings remain in `CategoryLandingPage.js`, stale Browserslist/caniuse-lite data remains, the homepage still lacks strong visual proof of industrial scale, current visuals do not yet fully communicate equipment, production lines, logistics, implementation, or project capability, category cards still need stronger future visual differentiation, the Inquiry Process is functional but visually dry, and final homepage quality still depends on the planned visual evidence strategy.
