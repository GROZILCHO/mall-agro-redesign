...
| ISS-023 | Route-pair mapping for cross-domain language switch is not defined. | Open | Route-pair config foundation is implemented in `src/lib/routes/routePairs.js` for the homepage and three approved category pairs. Keep open because language switcher UI and Romanian route wrappers remain pending. |
