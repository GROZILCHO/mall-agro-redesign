...
| ISS-009 | Domain localization strategy is not fully implemented. | Open | English baseline and Romanian category placeholder routes exist, but Romanian homepage/root behavior, language switcher UI, domain runtime behavior, and full localized metadata/content remain pending. |
...
| ISS-023 | Route-pair mapping for cross-domain language switch is not defined. | Open | Route-pair config foundation is implemented in `src/lib/routes/routePairs.js` for the homepage and three approved category pairs. Romanian category route targets now exist as placeholders. Keep open because language switcher UI and Romanian homepage/root behavior remain pending. |
