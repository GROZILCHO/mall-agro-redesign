...
| ISS-023 | Route-pair mapping for cross-domain language switch is not defined. | Open | Fallback behavior is approved: hide or disable unavailable language options when no approved equivalent exists. Route-pair source implementation and language switcher UI remain pending. |
